...
def main(symbols, username, password, force_unregistered=False, expire_after=0):#'00:15:00.0'):
...
""")
    ...
    # print(data.columns)
    # print(data.index)
    trianglepairs = ['GBP/USD', 'USD/JPY', 'GBP/JPY']

    l = []
    r = []
    d = []
    for x in trianglepairs:
        df = data[data.index == x]
        op = df.Open
        md = df.Date.iloc[0]
        d += [str(md)]
        print(str(md))
        symbols = x.split("/")
        conv = (1/op)[0]
        print("1",symbols[0],"=", op[0], symbols[1], "Conversion: ", conv)
        r += [str(conv)]
        l += symbols

    payload = f7(l)
    payload = ','.join(payload) + ',' + ','.join(r) + ',' + ','.join(d)
    print(payload)
    # for x in 1000000
    my_server.lpush('images', payload)

if __name__ == "__main__":
    for z in range(0,10):
        print(z)
        u, p = _init_credentials()
        main(SYMBOLS_NOT_AUTH, u, p)
        time.sleep(10)
